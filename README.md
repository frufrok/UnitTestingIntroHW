...
Используйте Pytest (для Python) или JUnit (для Java) для
написания тестов, которые проверяют правильность работы программы. 
...
метод getBiggerListIndex() возвращает значение, отличное от 0, 1 или 2, 
что не предусмотрено его реализацией. В случае, если метод будет изменен,
такое исключение позволит выявить ошибку расчета.

Для покрытия этой строки тестами необходимо сделать Spy-заглушку, но
Spy-заглушки имеют ограничения: они не могут работать с final-классами, 
к которым относятся записи (records), а именно с помощью записи реализован
основной функционал программы.

Передача невалидных данных (пустые списки или null-значения) в конструктор
записи ListComparator вызывает исключения, для каждого из которого написан
тест.

В случае, если инициализация объекта прошла без вызова исключений, ошибка
может возникнуть только в случае порчи данных (например, внешний объект 
удаляет все элементы из исходного списка (или списков)), для чего выполняется
дополнительная проверка на случай пустого (или пустых) списков, для которой
также предусмотрен тест.
